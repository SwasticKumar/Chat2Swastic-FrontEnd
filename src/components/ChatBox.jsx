...
import {
  ...
  sentMessageRequest,
} from "../apiRequest/messageRequest";
        ...
        >
          <div className="flex justify-between bg-[#f0f2f5] px-2 lg:px-4 py-2 border-b-2">
            <div className="flex flex-none space-x-1 md:space-x-3 items-center">
              <svg
                onClick={() => store.dispatch(setSelectUser(null))}
                className="p-1 rounded-full lg:hidden block"
              ...
              <div className="relative">
                {selectUser.isGroupChat ? (
                  <GroupIcon
                    className={`${
                      selectUser.isGroupChat && "bg-white"
                    } object-cover w-18 h-18 rounded-full`}
                  />
                ) : (
                  <img
                    className={`${
                ...
                <span className="font-bold text-black truncate">
                  {
                    selectUser.isGroupChat
                      ? selectUser.chatName
                      : getSender(selectUser?.users, getUserDetails())
                          .firstname +
                        " " +
                        getSender(selectUser?.users, getUserDetails()).lastname //getSender(selectUser.users, getUserDetails())//
                  }
                </span>
                {selectUser.isGroupChat ? (
                  <span className="truncate">
                    {selectUser.users.map((u, i) => (
                      <span
                        key={i}
                        className="text-xs truncate md:text-sm text-[#777e83]"
                      >
                        {u.firstname + ", "}
                      </span>
                    ))}
