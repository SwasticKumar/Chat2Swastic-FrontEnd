...
const MyChat = ({ myChats, dispatch }) => {
  ...
  return (
    <Fragment>
      <div className="flex flex-col border-r border-neutral-700 w-full h-[89.5vh]">
        <div className="flex justify-between items-center bg-[#f0f2f5] w-full h-[60px] px-4 py-2 border-b">
          <div className="flex items-center space-x-8 ms-auto">
            <svg
              onClick={() => dispatch({ type: "SHOWGUS" })}
              width="27"
              height="27"
        ...
        <div className="flex space-x-2 items-center w-full h-[60px] p-2 border-b ">
          <div className="flex-grow flex items-center space-x-2 bg-gray-200 py-1 px-4 rounded-md">
            ...
            <input
              onKeyUp={TextSearch}
              type="search"
              className="focus:outline-none bg-gray-200 w-full text-gray-700  text-xs"
        ...
        <div className="flex flex-col overflow-y-auto cursor-pointer w-full h-full">
          {myChats?.map((items, i) => {
            return (
              <button
                key={i}
                onClick={accessChatMsg.bind(this, items)}
                className={`flex items-center w-full px-4 py-2 border-b transition-colors duration-200 gap-x-3 ${
                  selectUser?._id === items._id
                    ? "bg-gray-200"
  ...
  {getOnline(items, onlineUsers, getUserDetails()) && (
    <span className="h-3 w-3 rounded-full bg-violet-500 absolute right-0.5 ring-2 ring-white -bottom-0.5"></span>
  )}
</div>
                ...
                <div className="flex justify-between w-full">
                  <div className="text-left rtl:text-right space-y-1 ">
                    ...
                    >
                      {items.isGroupChat
                        ? items.chatName
                        : getSender(items.users, getUserDetails()).firstname +
                          " " +
                          getSender(items.users, getUserDetails()).lastname}
                    </h1>
                    <div
                      className={`${
                        notifications.find((n) => n.chat._id === items._id)
                          ? "text-[#00a884]"
                          : "text-gray-600"
                    ...
                    >
                      {items.latestMessage &&
                        (items.isGroupChat ||
                          items.latestMessage?.sender?._id ===
                            getUserDetails()._id) && (
                          <span className="font-semibold text-sm">{`${
                            items.latestMessage?.sender?._id ===
                            getUserDetails()._id
                              ? "You"
                              : items.latestMessage?.sender?.firstname
                          }: `}</span>
                        )}
                  ...
                  <div className="flex flex-col flex-none text-xs items-end space-y-2 w-20 pr-2">
                    ...
                    >
                      {moment(items.latestMessage?.createdAt).format(
                        "DD/MM/YYYY"
                      )}
                    </p>
